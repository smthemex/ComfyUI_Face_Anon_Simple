...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        referencenet_states: Optional[torch.FloatTensor] = None,
        ...
        is_referencenet_states_available = (
            False if referencenet_states is None else True
        )
        if is_referencenet_states_available:
            referencenet_states = referencenet_states.to(hidden_states.dtype)
            concatenated_hidden_states = torch.cat(
                [norm_hidden_states, referencenet_states], dim=1
            )
            ...
            referencenet_states = norm_hidden_states
            attn_output = self.attn1(
                norm_hidden_states,
        ...
        if is_referencenet_states_available:
            # Split the tensor into three chunks along the second dimension
            chunks = torch.chunk(attn_output, chunks=3, dim=1)

            # Select the first chunk
            first_half = chunks[0]

            hidden_states = first_half + hidden_states
        else:
        ...
        return hidden_states, referencenet_states
